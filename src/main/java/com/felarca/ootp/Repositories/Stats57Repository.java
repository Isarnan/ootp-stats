...
public interface Stats57Repository extends JpaRepository<Stats57, Integer> {
	...
	// Removing title from group by.  cid should be enough
	@Query("select new com.felarca.ootp.domain.Hitter(s.cid as cid, 'fix this' as card, group_concat(s.pos) AS pos, avg(s.ovr) as ovr, sum(s.pa) as pa, sum(s.b_pitches) as b_pitches, sum(s.singles) as singles, sum(s.doubles) as doubles, sum(s.triples) as triples, sum(s.hr) as hr, sum(s.bb) as bb, sum(s.ibb) as ibb, sum(s.hp) as hp, sum(s.innings) as ip, sum(s.er) as er, sum(s.p_singles) as p_singles, sum(s.p_doubles) as p_doubles, sum(s.p_triples) as p_triples, sum(s.p_homeruns) as p_homeruns, sum(s.p_bb) as p_bb, sum(s.p_ibb) as p_ibb, sum(s.p_hp) as p_hp, sum(s.ab) as ab, sum(s.sb) as sb, sum(s.cs) as cs, sum(k) as k, coalesce(sum(g),0) as g, coalesce(sum(gs),0) as gs, sum(p_games) as p_games, sum(p_gamesstarted) as p_gamesstarted, sum(pi) as p_pitches, sum(fb) as p_fb, sum(gb) as p_gb, coalesce(sum(p_runs),0) as p_runs) "
			+ " from Stats57 s " + " where s.cid = :cid and tournament_type = :tournament_type and function('str_to_date', s.date, '%Y-%m-%d %H:%i:%s.%f') BETWEEN :after AND :before group by s.cid")
	...
	@Query("select new com.felarca.ootp.domain.Hitter(s.cid as cid, s.title as card, group_concat(s.pos) AS pos, avg(s.ovr) as ovr, sum(s.pa) as pa, sum(s.b_pitches) as b_pitches, sum(s.singles) as singles, sum(s.doubles) as doubles, sum(s.triples) as triples, sum(s.hr) as hr, sum(s.bb) as bb, sum(s.ibb) as ibb, sum(s.hp) as hp, sum(s.innings) as ip, sum(s.er) as er, sum(s.p_singles) as p_singles, sum(s.p_doubles) as p_doubles, sum(s.p_triples) as p_triples, sum(s.p_homeruns) as p_homeruns, sum(s.p_bb) as p_bb, sum(s.p_ibb) as p_ibb, sum(s.p_hp) as p_hp, sum(s.ab) as ab, sum(s.sb) as sb, sum(s.cs) as cs, sum(k) as k, coalesce(sum(g),0) as g, coalesce(sum(gs),0) as gs, sum(p_games) as p_games, sum(p_gamesstarted) as p_gamesstarted, sum(pi) as p_pitches, sum(fb) as p_fb, sum(gb) as p_gb, coalesce(sum(p_runs),0) as p_runs) "
	+ " from Stats57 s " + " where s.cid = :cid and tournament_type = :tournament_type and function('str_to_date', s.date, '%Y-%m-%d %H:%i:%s.%f') BETWEEN :after AND :before AND s.p_gamesstarted > 0 group by s.cid, s.title")
	public Hitter getStarter(@Param("cid") int cid, @Param("tournament_type") String tournament_type, @Param("before") LocalDateTime before, @Param("after") LocalDateTime after);
	
	@Query("select new com.felarca.ootp.domain.Hitter(s.cid as cid, 'Get RP' as card, group_concat(s.pos) AS pos, avg(s.ovr) as ovr, sum(s.pa) as pa, sum(s.b_pitches) as b_pitches, sum(s.singles) as singles, sum(s.doubles) as doubles, sum(s.triples) as triples, sum(s.hr) as hr, sum(s.bb) as bb, sum(s.ibb) as ibb, sum(s.hp) as hp, sum(s.innings) as ip, sum(s.er) as er, sum(s.p_singles) as p_singles, sum(s.p_doubles) as p_doubles, sum(s.p_triples) as p_triples, sum(s.p_homeruns) as p_homeruns, sum(s.p_bb) as p_bb, sum(s.p_ibb) as p_ibb, sum(s.p_hp) as p_hp, sum(s.ab) as ab, sum(s.sb) as sb, sum(s.cs) as cs, sum(k) as k, coalesce(sum(g),0) as g, coalesce(sum(gs),0) as gs, sum(p_games) as p_games, sum(p_gamesstarted) as p_gamesstarted, sum(pi) as p_pitches, sum(fb) as p_fb, sum(gb) as p_gb, coalesce(sum(p_runs),0) as p_runs) "
+ " from Stats57 s " + " where s.cid = :cid and tournament_type = :tournament_type and function('str_to_date', s.date, '%Y-%m-%d %H:%i:%s.%f') BETWEEN :after AND :before AND s.p_gamesstarted = 0 group by s.cid")
	public Hitter getReliever(@Param("cid") int cid, @Param("tournament_type") String tournament_type, @Param("before") LocalDateTime before, @Param("after") LocalDateTime after);
